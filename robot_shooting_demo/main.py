...
def gimbal_control(gimbal_instance, x_error, y_error, STOP, DETECTED):
    ...
    while True:
        if not DETECTED.value:
            continue
        gimbal_instance.drive_speed(pitch_speed = pitch_influence * pitch_controller.getparams(y_error.value), yaw_speed = yaw_influence * yaw_controller.getparams(x_error.value))
        if STOP.value:
            break

def gimbal_control_t():
    global gimbal_instance, x_error, y_error, STOP, DETECTED
...
if __name__ == "__main__":
    ...
    # target_centering_process = Process(target = gimbal_control, args = (gim_inst, x_error, y_error, STOP, DETECTED))
    target_centering_process = threading.Thread(target = gimbal_control_t, args = ())
    ...
    # target_centering_process.start()
    
    # target_centering_process.join()
